...
_, msa_seqs = load_gzdata('data/training_data/luxafilt_llmsa_train_sol0.fa.gz', one_hot=False)
_, val_msa_seqs = load_gzdata('data/training_data/luxafilt_llmsa_val_sol0.fa.gz', one_hot=False)
...
callbacks=[CSVLogger('output/logs/msavae_sol0.csv')]

if save_all_epochs:
    callbacks.append(ModelCheckpoint('output/weights/msavae_sol0'+'.{epoch:02d}-{luxa_errors_mean:.2f}.hdf5',
...
if not save_all_epochs:
  model.save_weights('output/weights/msavae_sol0.h5')
