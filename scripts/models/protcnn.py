...
class BaseProtVAE:
    ...
    def generate_variants_luxA(self, num_samples, posterior_var_scale=1., temperature=0.,
        ...
        target_len_for_seq = 0

        if isinstance(self.E.input_shape[1], int): 
            target_len_for_seq = self.E.input_shape[1]
        else:
            target_len_for_seq = self.E.input_shape[0][1]

        luxa_oh = to_one_hot(right_pad([luxa_seq], target_len_for_seq))
